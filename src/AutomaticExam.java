...
public class AutomaticExam implements Examable{
	...
	//constructor
	public AutomaticExam(QuestionPool questionPool) {
	...
	public void createExam(QuestionPool questionPool) {
		...
		try {
			...
			SelectionQuestion[] questions = questionPool.getQuestions();
				...
				if (!(questions[i] instanceof OpenQuestion)) {
					...
					for (int j = 0; j < 10; j++) {
						if ((questions[i].getAnswers()[j] != null)
							...
							if (questions[i].getAnswers()[10].getCorrect()) {
					...
					pwExam.println("* " + questions[i].getAnswers()[11].getContent() + "\n");
					pwSolution.println("* " + questions[i].getAnswers()[11].toString() + "\n");
				} else {
